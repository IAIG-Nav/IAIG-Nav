"""
Training Script for AIG-Nav (Action-conditioned Interaction Graph for Social Navigation)

This script trains the AIG-Nav model for social robot navigation.

Key Innovation Being Trained:
- Policy learns to choose actions based on PREDICTED FUTURE INTERACTIONS
- Pedestrian trajectories are predicted conditioned on robot's action
- Edges in the graph represent collision probabilities, not current distances

Usage:
    python scripts/train_aig_nav.py --render       # With visualization
    python scripts/train_aig_nav.py                # Training only
"""
...
from aig_nav_core import AIGNav, AIGNavReplayBuffer, PygameNavEnvAIGNav
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train AIG-Nav (SAC)")
    ...
    parser.add_argument("--log-dir", default="experiments/runs/aig_nav_sac")
    ...
    method_name = "IAIG-Nav" if args.use_iterative_eq else "AIG-Nav"
    print("=" * 60)
    print(f"{method_name} Training (SAC)")
