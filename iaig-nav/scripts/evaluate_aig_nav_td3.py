"""
Evaluation script for AIG-Nav-TD3.
...
def apply_prediction_noise(encoder, noise_std: float, seed: int):
    original_fn = encoder.predict_ped_trajectories
    generators: Dict[str, torch.Generator] = {}

    def noisy_predict(*args, **kwargs):
        preds = original_fn(*args, **kwargs)
        if noise_std <= 0.0 or preds.numel() == 0:
            return preds
        key = str(preds.device)
        gen = generators.get(key)
        if gen is None:
            gen = torch.Generator(device=preds.device) if preds.device.type == "cuda" else torch.Generator()
            if seed is not None:
                gen.manual_seed(seed)
            generators[key] = gen
        noise = torch.randn(preds.shape, dtype=preds.dtype, device=preds.device, generator=gen) * noise_std
        return preds + noise
...
def restore_prediction_noise(encoder, original_fn):
    encoder.predict_ped_trajectories = original_fn
...
def finalize_prediction(
    ...
    pred_trajs = pred_item["pred_trajs"]
    pred_coll = pred_item["pred_coll"]
    ...
    actual_trajs = np.stack(actual_list, axis=1)
    ...
    robot_positions = np.stack(robot_positions, axis=0)
    ped_positions = np.stack(ped_positions, axis=0)
    distances = np.linalg.norm(ped_positions - robot_positions[:, None, :], axis=-1)
    labels = (distances < collision_dist).any(axis=0).astype(np.float32)
...
def compute_safety_metrics(state_history, collision_dist, near_miss_dist, pred_horizon, dt):
    min_sep = float("inf")
    near_miss_count = 0
    ttc_values = []
...
def evaluate_noise_level(env, model, episodes, max_steps, pred_horizon, noise_std, collision_dist, near_miss_dist, dt, scenarios, seed_base):
    ade_sum = 0.0
    ade_count = 0
    fde_sum = 0.0
    fde_count = 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate AIG-Nav-TD3 with predictive metrics")
    ...
    parser.add_argument("--log-dir", default="experiments/results/aig_nav_td3_eval")
    ...
    for idx, noise_std in enumerate(noise_stds):
        ...
        orig_fn_infer = apply_prediction_noise(model.aig_encoder_infer, noise_std, seed=idx)
        ...
        restore_prediction_noise(model.aig_encoder_infer, orig_fn_infer)
